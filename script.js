...
async function handleLeadSubmit(event){event.preventDefault();const form=event.target;const fullName=form.querySelector('input[data-placeholder-vi="Họ và tên"]').value.trim();const phone=form.querySelector('input[data-placeholder-vi="Số điện thoại / Zalo"]').value.trim();const business=form.querySelector('input[data-placeholder-vi="Bạn đang kinh doanh sản phẩm/dịch vụ gì?"]').value.trim();const service=form.querySelector('select').value;const note=form.querySelector('textarea').value.trim();const btn=form.querySelector('button[type="submit"]');const message=`🔥 LEAD MỚI TỪ VIET GROWTH EU
...
const observer=new IntersectionObserver(entries=>{entries.forEach(e=>{if(e.isIntersecting)e.target.classList.add('show')})},{threshold:.15});document.querySelectorAll('.fade-up').forEach(el=>observer.observe(el));